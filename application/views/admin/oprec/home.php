...
<main id="main-container">
	...
	<!-- Hero -->
	<div class="bg-image"
		style="background-image: url(<?= base_url('assets/public/images/bangkinglab_ug_video.png') ?>);">
		<div class="bg-black-op">
			<div class="content content-top text-center">
				<div class="py-50">
					<h1 class="font-w700 text-white mb-10">Aplikasi Perbankan</h1>
				</div>
			</div>
		</div>
	</div>
	...
	if ($this->session->userdata('sebagai') == 'calonasisten' || $this->session->userdata('sebagai') == 'calonprogrammer' || $this->session->userdata('sebagai') == 'admin') { ?>
		...
		<div class="content">
			...
			if ($lock_ujian == 'ya') {
				...
				<nav class="breadcrumb bg-success push">
					<span class="breadcrumb-item active"><i class="fa fa-fw fa-check"></i> Selamat
						<?= ucfirst($this->auth_libs->user_login()->nama); ?>, anda lulus <b>seleksi berkas</b></span>
				</nav>
				<?php
			} else if ($lock_ujian == 'tidak') {
				?>
					<nav class="breadcrumb bg-danger push">
						<span class="breadcrumb-item active"><i class="fa fa-fw fa-times"></i> Maaf
						<?= ucfirst($this->auth_libs->user_login()->nama); ?>, anda tidak lulus <b>seleksi berkas</b></span>
					</nav>
				<?php
			}
			?>
			...
			if ($lock_wawancara == 'ya') {
				...
				<nav class="breadcrumb bg-success push">
					<span class="breadcrumb-item active"><i class="fa fa-fw fa-check"></i> Selamat
						<?= ucfirst($this->auth_libs->user_login()->nama); ?>, anda lulus <b>Tahap Ujian</b></span>
				</nav>
				<?php
				...
				?>
					<nav class="breadcrumb bg-danger push">
						<span class="breadcrumb-item active"><i class="fa fa-fw fa-times"></i> Maaf
						<?= ucfirst($this->auth_libs->user_login()->nama); ?>, anda tidak lulus <b>Tahap Ujian</b></span>
					</nav>
				<?php
			}
			?>
			...
			if ($lock_staff == 'ya') {
				...
				<nav class="breadcrumb bg-success push">
					<span class="breadcrumb-item active"><i class="fa fa-fw fa-check"></i> Selamat
						<?= ucfirst($this->auth_libs->user_login()->nama); ?>, anda lulus <b>Tahap 2 - Tutor, Wawancara, dan
							Pemrograman (Programmer)</b></span>
				</nav>
				<?php
				...
				?>
					<nav class="breadcrumb bg-danger push">
						<span class="breadcrumb-item active"><i class="fa fa-fw fa-times"></i> Maaf
						<?= ucfirst($this->auth_libs->user_login()->nama); ?>, anda tidak lulus <b>Tahap 2 - Tutor, Wawancara,
								dan
								Pemrograman (Programmer)</b></span>
					</nav>
				<?php
			...
			if ($lock_finish == 'ya') {
				...
				<nav class="breadcrumb bg-success push">
					<span class="breadcrumb-item active"><i class="fa fa-fw fa-check"></i> Selamat
						<?= ucfirst($this->auth_libs->user_login()->nama); ?>, anda lulus <b>Tahap 3 - Wawancara
							Staff</b></span>
				</nav>
				<?php
				...
				?>
					<nav class="breadcrumb bg-danger push">
						<span class="breadcrumb-item active"><i class="fa fa-fw fa-times"></i> Maaf
						<?= ucfirst($this->auth_libs->user_login()->nama); ?>, anda tidak lulus <b>Tahap 3 - Wawancara
								Staff</b></span>
					</nav>
				<?php
			}
						...
						<tbody>
							<tr class="table-active">
								...
								if ($lock_wawancara == 'ya') {
									...
									<th>
										<a href="<?= base_url('oprec/tutor/') ?>">Tahap 2 - Tutor, Wawancara Asisten dan
											Programmer, dan Tes Pemrograman (Programmer)</a>
									</th>
									<?php
								} else {
									?>
									...
									<th>
										<a>Tahap 2 - Tutor, Wawancara Asisten dan Programmer, dan Tes Pemrograman
											(Programmer)</a>
									</th>
									<?php
								}
		...
		if ($this->session->userdata('sebagai') == 'asisten' || $this->session->userdata('sebagai') == 'programmer' || $this->session->userdata('sebagai') == 'admin') { ?>
											...
											<tr>
												<td class="text-center">
													<img class="img-avatar img-avatar48"
														src="<?= base_url('assets/upload/foto/') ?><?= $aslab['nama'] ?>/<?= $aslab['foto'] ?>"
												...
												<td class="text-center font-w600"><a
														href="https://wa.me/+62<?= $aslab['telepon'] ?>"
														target="_blank"><?php echo $aslab['telepon'] ?></a></td>
												<td class="text-center">
													<?php
												...
												<td class="text-center">
													...
													if ($aslab['seleksi_berkas'] == 'ya' && $aslab['seleksi_ujian'] == 'ya' && $aslab['seleksi_wawancara'] != 'ya' && $aslab['seleksi_staff'] != 'ya') {
														?>
														<div class="font-size-sm text-muted text-black">Tahap Tutor, Wawancara, dan
															Pemrograman (Programmer)</div>
														<?php
													}
													?>
													<?php
												...
												<td class="text-center">
													<div class="btn-group">
														<a type="button" href="<?= base_url('oprec/berkas/') ?>"
															class="btn btn-sm btn-outline-primary" data-toggle="modal"
															data-target="#berkas_asisten<?= $aslab['id_user'] ?>"
															title="Detail">
															<i>Detail</i>
														</a>
													</div>
			...
			<!-- ini isi homepage nya kalo mau beda beda isi nya -->
							...
							<tbody>
								...
								foreach ($staff as $aslab): ?>
									<tr>
										...
										<td class="font-w600"><b>
												<?= $aslab['nama'] ?>
											</b></td>
										<td class="font-w600"><b>
												<?= $aslab['npm'] ?>
											</b></td>
										<td class="text-center font-w600"><a href="https://wa.me/+62<?= $aslab['telepon'] ?>"
												target="_blank"><?php echo $aslab['telepon'] ?></a></td>
										<?php
										if ($aslab['sebagai'] == 'calonasisten') {
											?>
										...
										<td class="text-center">
											<div class="btn-group">
												<a type="button" href="<?= base_url('oprec/berkas/') ?>"
													class="btn btn-sm btn-outline-primary" data-toggle="modal"
													data-target="#berkas_asisten<?= $aslab['id_user'] ?>" title="Detail">
													<i>Detail</i>
												</a>
											</div>
										</td>
...
foreach ($asisten as $detail): ?>
	<div class="modal fade" id="berkas_asisten<?= $detail['id_user'] ?>" tabindex="-1" role="dialog"
		aria-labelledby="berkas_asiste" aria-hidden="true">
										...
										<div class="col-md-12">
											<a class="block text-center" href="javascript:void(0)">
												<div class="block-content block-content-full">
													...
													if ($detail['seleksi_berkas'] == 'ya' && $detail['seleksi_ujian'] == 'ya' && $detail['seleksi_wawancara'] != 'ya' && $detail['seleksi_staff'] != 'ya') {
														?>
														<div class="font-size-sm text-muted text-black">Tahap Tutor, Wawancara,
															dan Pemrograman (Programmer)</div>
														<?php
													}
													?>
													<?php
													...
													if ($detail['seleksi_berkas'] == 'ya' && $detail['seleksi_ujian'] = 'ya' && $detail['seleksi_wawancara'] == 'ya' && $detail['seleksi_staff'] == 'ya') {
														?>
														<div class="font-size-sm text-muted text-black">Lulus Semua Seleksi
														</div>
														<?php
													}
													?>
												...
												<div class="block-content block-content-full bg-gd-dusk">
													<img class="img-avatar img-avatar48"
														src="<?= base_url('assets/upload/foto/') ?><?= $detail['nama'] ?>/<?= $detail['foto'] ?>"
														width="200" height="200" alt="">
												</div>
												<div class="block-content block-content-full">
												...
												<div
													...
													<span class="font-w600 font-size-sm text-elegance">
														<?= $detail['email'] ?>
													</span>
												</div>
											</a>
										...
										<tbody>
											...
											<tr>
												<td>
													...
													if ($detail['cv'] == '') {
														...
														?>
														<img
															src="https://img.icons8.com/external-tal-revivo-bold-tal-revivo/24/26e07f/external-approved-checkmark-symbol-to-verify-the-result-basic-bold-tal-revivo.png" />
														<?php
													}
													?>
													<a href="<?= base_url('assets/upload/dokumen/') ?><?= $detail['nama'] ?>/<?= $detail['cv'] ?>"
														target="_blank">CV</a>
												</td>
											</tr>
											<tr>
												<td>
													...
													?>
													<a href="<?= base_url('assets/upload/dokumen/') ?><?= $detail['nama'] ?>/<?= $detail['surat_lamaran'] ?>"
											...
											<tr>
												<td>
													...
													?>
													<a href="<?= base_url('assets/upload/dokumen/') ?><?= $detail['nama'] ?>/<?= $detail['rangkuman_nilai'] ?>"
														target="_blank">Rangkuman Nilai</a>
												</td>
											</tr>
											<tr>
												<td>
													...
													?>
													<a href="<?= base_url('assets/upload/dokumen/') ?><?= $detail['nama'] ?>/<?= $detail['krs_upload'] ?>"
														target="_blank">KRS Aktif</a>
												</td>
											</tr>
											<tr>
												<td>
													...
													?>
													<a href="<?= base_url('assets/upload/dokumen/') ?><?= $detail['nama'] ?>/<?= $detail['ktm_upload'] ?>"
														target="_blank">KTM</a>
												</td>
											</tr>
											<tr>
												<td>
													...
													?>
													<a href="<?= base_url('assets/upload/dokumen/') ?><?= $detail['nama'] ?>/<?= $detail['sertifikat'] ?>"
									...
									<div class="block-content">
										<form action="" method="post" onsubmit="return false;">
											<div class="form-group">
												...
												<input type="text" name="nama" class="form-control" id="example-nf-nama"
													placeholder="<?= $detail['nama'] ?>">
											</div>
											<div class="form-group">
												<label for="example-nf-npm">NPM</label>
												<input type="text" name="npm" class="form-control" id="example-nf-npm"
													placeholder="<?= $detail['npm'] ?>">
											</div>
											<div class="form-group">
												<label for="example-nf-kelas">Kelas</label>
												<input type="text" name="kelas" class="form-control" id="example-nf-kelas"
													placeholder="<?= $detail['kelas'] ?>">
											</div>
											<div class="form-group">
												<label for="example-nf-email">Email</label>
												<input type="email" name="email" class="form-control" id="example-nf-email"
													placeholder="<?= $detail['email'] ?>">
											</div>
											<div class="form-group">
												<label for="example-nf-telepon">No. Telepon</label>
												<input type="text" name="telepon" class="form-control"
													id="example-nf-telepon" placeholder="<?= $detail['telepon'] ?>">
											</div>
											<div class="form-group">
											...
											<div class="form-group">
												...
												<input type="text" name="angkatan" class="form-control"
													id="example-nf-angkatan" placeholder="<?= $detail['angkatan'] ?>">
											</div>
											<div class="form-group">
												<label for="example-nf-ttl">Tempat & Tanggal Lahir</label>
												<input type="text" name="ttl" class="form-control" id="example-nf-ttl"
													placeholder="<?= $detail['lahir'] ?>">
											</div>
											<div class="form-group">
												<label class="col-12" for="example-alamat-input">Alamat</label>
												<textarea type="text" name="alamat" class="form-control"
													id="example-alamat-input" rows="6"
